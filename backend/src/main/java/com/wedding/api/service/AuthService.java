...
public class AuthService {
    ...
    public AuthResponse register(String email, String password, String verificationCode) {
        ...
        EmailVerificationService.Result verifyResult = emailVerificationService.verifyCode(normalizedEmail,
                verificationCode);
        ...
        if (existingUser != null) {
            if (existingUser.getProvider() != null && !existingUser.getProvider().isBlank()) {
                return AuthResponse
                        .fail("이미 " + providerLabel(existingUser.getProvider()) + "로 가입된 이메일입니다. 소셜 로그인으로 이용해 주세요.");
            }
    ...
    public void deleteAccount(String userId) {
        ...
        // 유저 삭제 (CascadeType.ALL에 의해 연관된 Invitation 등도 삭제됨)
    ...
    public AuthResponse login(String email, String password) {
        ...
        return AuthResponse.ok(accessToken, refreshToken,
    ...
    private AuthResponse upsertSocialUserAndLogin(String provider, String providerId, String email) {
        ...
        try {
            ...
            e.printStackTrace();
            return AuthResponse.fail("유저 정보 처리 중 오류가 발생했습니다: " + e.getMessage());
        }
        ...
        try {
            ...
            try {
                ...
                e.printStackTrace();
                return AuthResponse.fail("리프레시 토큰 생성 실패: " + e.getMessage());
            }
    ...
    private NaverVerifyResult verifyNaverAuthorizationCode(String code, String state, String redirectUri) {
        if (code == null || code.isBlank())
            return NaverVerifyResult.fail("네이버 code 값이 없습니다.");
        if (state == null || state.isBlank())
            return NaverVerifyResult.fail("네이버 state 값이 없습니다.");
        if (redirectUri == null || redirectUri.isBlank())
            return NaverVerifyResult.fail("네이버 redirectUri 값이 없습니다.");
        if (naverClientId == null || naverClientId.isBlank())
            return NaverVerifyResult.fail("서버 NAVER_CLIENT_ID 설정이 비어 있습니다.");
        if (naverClientSecret == null || naverClientSecret.isBlank())
            return NaverVerifyResult.fail("서버 NAVER_CLIENT_SECRET 설정이 비어 있습니다.");
        ...
        try {
            ...
            HttpResponse<String> tokenResp = HttpClient.newHttpClient().send(tokenReq,
            ...
            if (accessToken.isBlank())
                return NaverVerifyResult.fail("네이버 access_token이 비어 있습니다.");

            HttpRequest profileReq = HttpRequest.newBuilder(URI.create("https://openapi.naver.com/v1/nid/me"))
                    .header("Authorization", "Bearer " + accessToken)
                    .GET()
                    .build();
            HttpResponse<String> profileResp = HttpClient.newHttpClient().send(profileReq,
                    HttpResponse.BodyHandlers.ofString());
            ...
            if (!"00".equals(resultCode)) {
                return NaverVerifyResult
                        .fail("네이버 사용자 조회 resultcode 비정상: " + resultCode + ", body=" + profileResp.body());
            }
            ...
            if (id.isBlank())
                return NaverVerifyResult.fail("네이버 사용자 id가 비어 있습니다.");
    ...
    private KakaoVerifyResult verifyKakaoAuthorizationCode(String code, String state, String redirectUri) {
        if (code == null || code.isBlank())
            return KakaoVerifyResult.fail("카카오 code 값이 없습니다.");
        if (state == null || state.isBlank())
            return KakaoVerifyResult.fail("카카오 state 값이 없습니다.");
        if (redirectUri == null || redirectUri.isBlank())
            return KakaoVerifyResult.fail("카카오 redirectUri 값이 없습니다.");
        ...
        try {
            ...
            if (kakaoClientSecret != null && !kakaoClientSecret.isBlank()) {
                bodyBuilder.append("&client_secret=")
                        .append(URLEncoder.encode(kakaoClientSecret, StandardCharsets.UTF_8));
            }
            ...
            String accessToken = stringOrEmpty(tokenMap.get("access_token"));
            if (accessToken.isBlank())
                return KakaoVerifyResult.fail("카카오 access_token이 비어 있습니다.");
            ...
            if (id.isBlank())
                return KakaoVerifyResult.fail("카카오 사용자 id가 비어 있습니다.");

...
public class AdminService {
    ...
    private static final Set<String> ALLOWED_AI_CONFIG_KEYS = Set.of(
            "theme", "textScale", "fontFamily", "bgColor", "subBgColor", "textColor", "pointColor", "titleColor",
            "nameColor", "dateColor", "messageColor",
            "sectionTitleColor", "calendarBgColor", "calendarDayColor", "calendarActiveColor", "buttonColor",
            "buttonTextColor", "footerColor",
            "saveTheDateText", "mainTitleText", "groomDisplayName", "brideDisplayName", "venueDisplayName",
            "heroVenueNameText", "invitationBodyText",
            "groomFatherText", "groomMotherText", "groomRelationText", "brideFatherText", "brideMotherText",
            "brideRelationText",
            "titleSize", "namesSize", "dateSize", "saveTheDateSize", "heroVenueNameSize", "heroDDaySize", "contentSize",
            "familyLineSize",
            "galleryTitleSize", "locationTitleSize", "locationVenueNameSize", "locationAddressSize",
            "navButtonTextSize", "accountTitleSize", "accountSubtitleSize",
            "accountToggleLabelSize", "accountHeaderSize", "accountInfoSize", "attendanceTitleSize",
            "attendanceDescSize", "guestbookTitleSize", "guestbookDescSize",
            "attendanceLabelSize", "attendanceOptionTextSize", "formPlaceholderSize", "calendarTitleSize",
            "calendarDaySize", "footerWeddingOfSize");
    ...
    private static final List<Pattern> BLOCKED_AI_PROMPT_PATTERNS = List.of(
            Pattern.compile("\\b(api\\s*key|apikey|secret|token|password|passwd|private\\s*key)\\b",
                    Pattern.CASE_INSENSITIVE),
            Pattern.compile("\\b(env|\\.env|credential|auth\\s*header|bearer)\\b", Pattern.CASE_INSENSITIVE),
            Pattern.compile("\\b(키\\s*알려|키\\s*보여|토큰\\s*알려|비밀번호\\s*알려|시크릿\\s*알려|환경변수\\s*보여)\\b",
                    Pattern.CASE_INSENSITIVE),
            Pattern.compile("\\b(hack|exploit|inject|bypass|jailbreak|prompt\\s*injection)\\b",
                    Pattern.CASE_INSENSITIVE));
    ...
    public List<Skin> getSkins() {
        return skinRepository.findAllByOrderByCreatedAtDesc();
    }
    ...
    public void updateSkin(String id, SkinRequest req) {
        ...
        if (req.getConfig() != null)
            skin.setConfig(sanitizeSkinConfigJson(req.getConfig()));
        if (req.getThumbnail() != null)
            skin.setThumbnail(req.getThumbnail());
    ...
    public Map<String, Object> generateSkinConfigByAi(SkinAiGenerateRequest req) {
        ...
        String template = req.getTemplate() == null || req.getTemplate().isBlank() ? "modern"
                : req.getTemplate().trim();
        ...
        for (Map.Entry<String, Object> e : raw.entrySet()) {
            ...
            if (!ALLOWED_AI_CONFIG_KEYS.contains(e.getKey()))
                continue;
            Object v = e.getValue();
            if (v == null)
                continue;
    ...
    private String resolveAllowedFontFamily(String raw) {
        ...
        for (String allowed : ALLOWED_FONT_FAMILIES) {
            ...
            if (normalizedAllowed.startsWith(normalizedInput + ","))
                return allowed;
            if (normalizedAllowed.contains(normalizedInput))
                return allowed;
            if (normalizedInput.contains(normalizedAllowed))
                return allowed;
        }
        return null;
    }
    ...
    public int applyDefaultThemeColors() {
        ...
        for (Skin skin : skinRepository.findAll()) {
            ...
            Map<String, String> defaults = "modern".equals(slug) ? modernDefaults
                    : "classic".equals(slug) ? classicDefaults : null;
            if (defaults == null)
                continue;
            try {
